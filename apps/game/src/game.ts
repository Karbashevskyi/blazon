/**
 * Blazon Game — Heraldry Quiz (i18n-aware)
 *
 * Three game modes:
 *  1. pick-name  — see a coat, pick the city name from 4 options
 *  2. pick-coat  — see a city name, pick the correct coat from 4 options
 *  3. type-name  — see a coat, type the city name
 */
...
/** Normalize for fuzzy comparison: lowercase, strip diacritics, keep alphanumeric. */
...
export function initGame(cityEntries: readonly BlazonLocality[], t: Translations): void {
  ...
  function updateScore(): void {
    ...
    if (streakWrap !== null && streakNum !== null) {
      ...
      if (show) streakNum.textContent = t.streakLabel(score.streak);
    }
  }
  ...
  function feedbackHtml(): string {
    ...
    const text = lastCorrect
      ? t.feedbackCorrect
      : t.feedbackWrong(`<strong>${escHtml(cityLabel(current))}</strong>`);
    return `<div class="game-feedback ${cls}" role="alert">${text}</div>`;
  }

  function nextBtnHtml(): string {
    return `<button class="btn btn-primary game-next-btn">${escHtml(t.next)}</button>`;
  }
  ...
  function renderPickName(): void {
    gameArea.innerHTML = `
      <div class="game-card" data-mode="pick-name">
        <p class="game-question">${escHtml(t.questionPickName)}</p>
  ...
  function renderPickCoat(): void {
    gameArea.innerHTML = `
      <div class="game-card" data-mode="pick-coat">
        <p class="game-question">${t.questionPickCoat(`<strong>${escHtml(cityLabel(current))}</strong>`)}</p>
  ...
  function renderTypeName(): void {
    gameArea.innerHTML = `
      <div class="game-card" data-mode="type-name">
        <p class="game-question">${escHtml(t.questionTypeName)}</p>
        <div class="game-coat-display">
          <div class="game-coat-svg">${getSvg(current)}</div>
        </div>
        ${
          answered
            ...
            : `<form class="game-type-form" id="game-type-form" novalidate>
                 ...
                 <div class="game-type-actions">
                   <button type="submit" class="btn btn-primary">${escHtml(t.check)}</button>
                   <button type="button" class="btn btn-secondary game-skip-btn">${escHtml(t.skip)}</button>
                 </div>
               </form>`
        }
      </div>`;
