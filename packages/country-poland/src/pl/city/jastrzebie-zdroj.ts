import type { CoatOfArms } from '@blazon/types';

/**
 * Herb Jastrzębie-Zdrój — Polish coat of arms (Jastrzębie-Zdrój)
 * @id pl-city-jastrzebie-zdroj
 */
const jastrzebieZdroj: CoatOfArms = {
  id: 'pl-city-jastrzebie-zdroj',
  name: 'Herb Jastrzębie-Zdrój',
  description: 'Coat of arms of Jastrzębie-Zdrój (Herb Jastrzębie-Zdrój), a city in Poland.',
  svg: '<svg\n   xmlns="http://www.w3.org/2000/svg"\n   id="svg2"\n   viewBox="-26.97 -36.05 593.39 793.04"><title>Herb Jastrzębie-Zdrój</title><metadata\n     id="metadata21"><rdf:RDF><cc:Work\n         rdf:about=""><dc:format>image/svg+xml</dc:format><dc:type\n           rdf:resource="http://purl.org/dc/dcmitype/StillImage" /></cc:Work></rdf:RDF></metadata><sodipodi:namedview\n     guidetolerance="10.0"\n     gridtolerance="10.0"\n     objecttolerance="10.0"\n     borderopacity="1.0"\n     bordercolor="#666666"\n     pagecolor="#ffffff"\n     id="base" /><defs\n     id="defs4"><marker\n       id="ArrowEnd"\n       viewBox="0 0 10 10"\n       refX="0"\n       refY="5"\n       markerUnits="strokeWidth"\n       markerWidth="4"\n       markerHeight="3"\n       orient="auto"><path\n         d="M 0 0 L 10 5 L 0 10 z"\n         id="path7" /></marker><marker\n       id="ArrowStart"\n       viewBox="0 0 10 10"\n       refX="10"\n       refY="5"\n       markerUnits="strokeWidth"\n       markerWidth="4"\n       markerHeight="3"\n       orient="auto"><path\n         d="M 10 0 L 0 5 L 10 10 z"\n         id="path10" /></marker></defs><g\n     id="g2167"><path\n       style="fill:#222222;fill-rule:evenodd;stroke:none"\n       d="M 0,0 L 547.988,0 L 547.988,428.031 C 534.636,725.699 8.3672,733.261 0,428.031 L 0,0"\n       id="path14" /><path\n       style="fill:#04a854;fill-opacity:1;fill-rule:evenodd;stroke:none"\n       d="M 8.5468,9.65198 L 539.445,9.65198 L 539.445,425.375 C 526.511,714.48 16.6562,721.828 8.5468,425.375 L 8.5468,9.65198 z M 207.308,429.32 L 171.984,419.449 C 155.257,436.152 125.023,383.156 179.46,378.687 C 187.945,376.796 188.66,369.05 181.597,361.523 C 124.91,284.296 54.3593,219.222 47.0039,136.246 C 58.2187,148.293 46.7539,111.461 48.0703,98.703 C 63.9179,109.687 44.4531,70.257 48.0703,55.793 C 80.6404,114.882 119.089,166.199 168.773,208.125 C 202.953,257.468 237.136,306.816 271.32,356.164 C 277.05,364.046 282.312,362.066 287.339,355.093 C 325.082,297.168 362.824,239.238 400.57,181.308 C 437.597,153.328 463.589,102.375 493.503,47.214 C 487.808,70.816 482.109,94.414 476.414,118.015 C 472.238,148.195 496.21,99.281 500.035,88.109 C 501.293,101.695 498.269,116.23 493.503,129.816 C 489.48,142.546 495.523,135.601 500.98,129.816 C 495.64,164.324 485.671,196.504 468.937,225.293 C 426.804,288.269 387.878,353.394 342.539,414.226 L 323.175,437.664 L 346.882,473.007 L 277.691,526.011 L 210.281,475.621 L 236.39,441.168 L 207.308,429.336"\n       id="path16" /><path\n       style="fill:#ffffff;fill-rule:evenodd;stroke:none"\n       d="M 282.043,378.961 C 256.859,405 246.867,449.199 282.437,449.957 C 321.41,450.132 298.66,396.55 282.043,378.961 z M 296.796,540.562 L 360.339,491.261 L 386.043,527.796 L 400.277,516.679 C 392.039,507.812 389.402,497.621 392.765,484.515 L 342.546,414.226 L 323.171,437.656 L 346.898,473 L 277.714,525.863 L 210.304,475.605 L 236.398,441.168 L 207.312,429.32 L 153.929,511.128 L 175.785,526.089 L 199.031,493.539 L 260.898,539.476 L 213.64,575.066 L 230.644,591.347 L 279.851,553.527 L 329.898,590.554 L 344.132,575.703 L 296.8,540.578"\n       id="path18" /></g></svg>',
  metadata: {
    countryCode: 'PL',
    type: 'municipal',
    level: 'city',
    region: 'Poland',
    city: 'Jastrzębie-Zdrój',
    updatedAt: '2026-05-08',
  },
  license: {
    spdx: 'CC0-1.0',
    name: 'Creative Commons Zero v1.0 Universal',
    url: 'https://creativecommons.org/publicdomain/zero/1.0/',
    source: 'https://commons.wikimedia.org/wiki/File:POL%20Jastrz%C4%99bie-Zdr%C3%B3j%20COA.svg',
  },
  tags: ['poland', 'jastrzebie-zdroj', 'city'],
} as const;

export { jastrzebieZdroj };
